 ...
 *
 * @param onNavigateToNextStep 다음 온보딩 화면 이동
...
fun EntryProviderScope<NavKey>.onboardingEntry(
    ...
    entry<OnboardingNavKey> { navKey ->
        Timber.e("## 현재 온보딩 단계 : ${navKey.step}")

        OnboardingScreen(
            currentStep = navKey.step,
            totalSteps = OnboardingNavKey.TOTAL_STEPS,
            onNext = {
                if (navKey.step < OnboardingNavKey.TOTAL_STEPS) {
                    // 다음 단계 이동
