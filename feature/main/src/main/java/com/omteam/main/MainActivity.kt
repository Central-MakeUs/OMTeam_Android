    ...
    LaunchedEffect(loginState) {
        when (val state = loginState) {
            is MainViewModel.LoginState.Success -> {
                val email = state.userInfo.email ?: "이메일 없음"
                Toast.makeText(
                    context,
                    "로그인 성공: ${state.userInfo.nickname} ($email)",
                    Toast.LENGTH_SHORT
                ).show()
            }
    ...
    Column(
        ...
        Text(
            text = when (val state = loginState) {
                ...
                is MainViewModel.LoginState.Success -> {
                    val email = state.userInfo.email ?: "이메일 없음"
                    "닉네임: ${state.userInfo.nickname}\n이메일: $email"
                }
                is MainViewModel.LoginState.Error -> "로그인 실패: ${state.message}"
            },
